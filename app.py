...
CHARGING_STOP_DURATION_MIN = 20  # minutes

def get_ors_key() -> str:
    # No st.secrets to avoid "No secrets found"
    return os.environ.get("OPENROUTESERVICE_API_KEY", DEFAULT_ORS_API_KEY)

# ------------------------------
# Weather / temperature effects (NEW)
# ------------------------------
...
def fetch_open_meteo_weather(lat: float, lon: float):
    ...
    temp_c = cw.get("temperature", None) if isinstance(cw, dict) else None
...
def battery_capacity_factor(temp_c: float) -> float:
    """Simple EV model: usable battery capacity decreases in cold/heat. factor in [0.75, 1.00]."""
    t = float(temp_c)
...
def battery_energy_multiplier(temp_c: float) -> float:
    """Simple EV model: extra energy due to internal losses at non-ideal temps. multiplier in [1.0, 1.35]."""
    t = float(temp_c)
...
def hvac_electric_power_kw(temp_c: float, intensity_pct: float, hvac_params: dict) -> float:
    """Return electrical HVAC power in kW (deterministic)."""
    ...
    if temp_c < (target - deadband):
        ...
        p_elec = 0.2 * cabin_factor  # gentle blower / maintenance
...
with st.expander("ℹ️ What's new and features", expanded=False):
    st.markdown(
        """
        **✨ Features included (all important):**
        - 👥 Passengers count + weight impact
        - 🌡️ HVAC model (vehicle-specific) + intensity
        - 🌦️ Weather (manual or Live Open-Meteo): temperature + rain
        - 🧪 Air density from temperature
        - 🌧️ Rain impact on rolling resistance (Crr multiplier)
        - 🔋 Battery temp effects: usable capacity factor + energy multiplier
        - 🔋 Charging planning: battery % at start and target at arrival + stops estimate
        - 💶 Trip cost estimate (€/kWh)
        - 🌍 CO₂ estimate (gCO₂/kWh presets or manual)
        - 🚦 Speed limits by segment (road type) + minimum delta
        - 🛣️ Improved intersection / slowdown detection
        - 🗺️ Route preview map
        - 📊 Comparison table + Energy vs Speed & Total time vs Speed charts
        - 🔧 ORS debug self-check (raw responses) in Debug mode
        """
    )
...
# HVAC profiles by vehicle
HVAC_PROFILES = {
    "Tesla Model 3": {"heat_type": "heat_pump", "max_heat_kw": 5.0, "max_cool_kw": 3.0, "cabin_factor": 1.00, "cop_heat": 2.8, "cop_cool": 2.6, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Tesla Model Y": {"heat_type": "heat_pump", "max_heat_kw": 5.5, "max_cool_kw": 3.2, "cabin_factor": 1.15, "cop_heat": 2.8, "cop_cool": 2.6, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Audi Q4 e-tron": {"heat_type": "heat_pump", "max_heat_kw": 5.5, "max_cool_kw": 3.2, "cabin_factor": 1.15, "cop_heat": 2.6, "cop_cool": 2.5, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "BMW iX3": {"heat_type": "heat_pump", "max_heat_kw": 5.8, "max_cool_kw": 3.3, "cabin_factor": 1.18, "cop_heat": 2.6, "cop_cool": 2.5, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Mercedes EQC": {"heat_type": "heat_pump", "max_heat_kw": 6.0, "max_cool_kw": 3.5, "cabin_factor": 1.25, "cop_heat": 2.4, "cop_cool": 2.4, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Volkswagen ID.4": {"heat_type": "heat_pump", "max_heat_kw": 5.5, "max_cool_kw": 3.2, "cabin_factor": 1.15, "cop_heat": 2.5, "cop_cool": 2.4, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Renault Zoe": {"heat_type": "heat_pump", "max_heat_kw": 4.0, "max_cool_kw": 2.5, "cabin_factor": 0.85, "cop_heat": 2.4, "cop_cool": 2.4, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "BMW i3": {"heat_type": "resistive", "max_heat_kw": 4.0, "max_cool_kw": 2.2, "cabin_factor": 0.80, "cop_heat": 1.0, "cop_cool": 2.3, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Nissan Leaf": {"heat_type": "resistive", "max_heat_kw": 4.5, "max_cool_kw": 2.5, "cabin_factor": 0.90, "cop_heat": 1.0, "cop_cool": 2.3, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Hyundai IONIQ 5": {"heat_type": "heat_pump", "max_heat_kw": 5.5, "max_cool_kw": 3.2, "cabin_factor": 1.10, "cop_heat": 2.6, "cop_cool": 2.5, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Kia EV6": {"heat_type": "heat_pump", "max_heat_kw": 5.5, "max_cool_kw": 3.2, "cabin_factor": 1.10, "cop_heat": 2.6, "cop_cool": 2.5, "target_cabin_c": 21.0, "deadband_c": 2.0},
    "Personnalisé": {"heat_type": "heat_pump", "max_heat_kw": 5.0, "max_cool_kw": 3.0, "cabin_factor": 1.00, "cop_heat": 2.5, "cop_cool": 2.5, "target_cabin_c": 21.0, "deadband_c": 2.0},
}
...
with st.sidebar:
    ...
    with st.expander("💡 Tip", expanded=False):
        ...
        **Factors that affect your consumption:**
        ...
        - 🏔️ **Topography**: Uphill segments increase consumption
        - 🏎️ **Speed**: Consumption rises ~exponentially with speed
        - 🌦️ **Weather**: rain increases rolling resistance; temp affects air density & battery
        """
        )
    ...
    hvac_params = HVAC_PROFILES.get(vehicle_profile, HVAC_PROFILES["Personnalisé"]).copy()
    ...
    # Defaults (overridden at run if live weather is enabled)
    temp_c = float(temp_c_manual)
    precip_mm_h = float(precip_mm_h_manual)

    rho_air_preview = air_density_from_temp_c(temp_c)
    rain_factor_preview = rolling_resistance_rain_factor(precip_mm_h)
    st.caption(f"Air density: {rho_air_preview:.3f} kg/m³ | Rain: {precip_mm_h:.1f} mm/h → Crr×{rain_factor_preview:.2f}")

    st.subheader("🔋 Battery temperature effects")
    use_battery_temp = st.checkbox("Model battery temp impact (EV)", value=True)
    batt_cap_fac_preview = battery_capacity_factor(temp_c) if use_battery_temp else 1.0
    batt_E_mult_preview = battery_energy_multiplier(temp_c) if use_battery_temp else 1.0
    st.caption(f"Usable capacity factor: {batt_cap_fac_preview:.2f} | Energy multiplier: {batt_E_mult_preview:.2f}")
    ...
    user_speed_limit = st.number_input("Max speed on route (km/h)", 50, 130, 110, 10, help="Fallback speed limit if API lacks info.")
    st.markdown("---")
    ...
    climate_intensity = st.slider("HVAC intensity (%)", 0, 100, 50, 10) if use_climate else 0
    ...
    use_segmented_speeds = st.checkbox("Speed limits by segment", value=True, help="Use road-type limits (motorway/city/etc.)")
    min_speed_delta = st.number_input(
        "Minimum speed delta (km/h)", 0, 50, 20, 5,
        help="Minimum speed = limit - delta (e.g., motorway 130, delta 20 → min 110)"
    ) if use_segmented_speeds else 0
    use_detailed_route = st.checkbox("Detailed route", value=True, help="Kept for compatibility (not strictly required)")
    debug_mode = st.checkbox("Debug mode", value=False, help="Display intermediate info + ORS diagnostics")
...
def seg_energy_and_time(distance_m, slope, speed_kmh, mass_kg, cda, crr, rho_air, eta_drive, regen_eff, aux_power_kw=0, **kwargs):
    ...
    # Forces
    F_aero = 0.5 * rho_air * float(cda) * v * v
    F_roll = float(crr) * float(mass_kg) * g * math.cos(math.atan(slope))
    F_grade = float(mass_kg) * g * math.sin(math.atan(slope))

    # Wheel power
    P_wheels = (F_aero + F_roll + F_grade) * v

    # Electrical power with regen
    if P_wheels >= 0:
        P_elec = P_wheels / max(float(eta_drive), 1e-6)
    else:
        P_elec = P_wheels * float(regen_eff)

    # Aux
    P_aux = float(aux_power_kw) * 1000.0
    ...
    # Battery temp multiplier (extra losses)
...
# ------------------------------
# Speed limits by segment + intersections
# ------------------------------
def get_speed_limit_by_road_type(road_type: str, user_max_speed: int = 130) -> int:
    ...
    for key, speed in speed_mapping.items():
        if road_type and key in str(road_type).lower():
            return speed
    return 50
...
def detect_intersections_improved(steps, detailed_segments, coords):
    ...
    for i, step in enumerate(steps):
        ...
        if any(k in instr for k in intersection_keywords):
        ...
        if step_type in [1, 2, 3, 4, 5, 6] and distance < 100:
...
def create_segmented_speeds(coords, steps, detailed_segments, candidate_speed: int, user_max_speed: int, min_speed_delta: int = 20):
    ...
    # cumulative distances along coords (approx)
    cumul_coord_dist = 0.0
    coord_distances = [0.0]
    for i in range(len(coords) - 1):
        lon1, lat1 = coords[i][0], coords[i][1]
        lon2, lat2 = coords[i + 1][0], coords[i + 1][1]
        segment_distance = _haversine_m(lon1, lat1, lon2, lat2)
    ...
    for i in range(len(coords) - 1):
        mid_dist = (coord_distances[i] + coord_distances[i + 1]) / 2.0
        ...
        if segment_found:
            ...
            if road_type:
                ...
                min_allowed_speed = max(30, speed_limit - int(min_speed_delta))
                final_speed = max(min_allowed_speed, min(int(candidate_speed), speed_limit))
            elif way_type:
                ...
                final_speed = int(candidate_speed)

            segmented_speeds.append(final_speed)
        else:
            segmented_speeds.append(int(candidate_speed))

    if not found_road_types:
        return [int(candidate_speed)] * (len(coords) - 1)

    return segmented_speeds if segmented_speeds else [int(candidate_speed)] * (len(coords) - 1)

# ------------------------------
# OpenRouteService / OSRM wrappers (keep robust fallbacks from original)
# ------------------------------
CITY_COORDS = {
    ...
    try:
        ...
        if "routes" in data and data["routes"]:
            ...
            if isinstance(geometry, dict) and geometry.get("type") == "LineString":
                ...
                # keep original polyline fallback if present
                try:
                    import polyline
...
def ors_elevation_along(coords, api_key):
    ...
    try:
        ...
        # If completely flat zeros, try extra fallbacks (keep original robustness)
        ...
        # Fallback 1: encoded polyline
        try:
            if len(coords) > 200:
                step2 = max(1, len(coords) // 200)
                reduced2 = coords[::step2]
            ...
            # Fallback 2: Open-Elevation (free)
            try:
                import numpy as _np
                ...
                # Interpolate back if downsampled
...
# ------------------------------
# Debug ORS check (keep from original)
# ------------------------------
def debug_ors_check(orig_text: str, dest_text: str, api_key: str):
    ...
    try:
        ...
        if not start or not end:
            st.warning("Directions skipped (missing geocode coords)")
            return
...
if run_btn:
    ...
    # Live weather override (Open-Meteo)
    temp_c_run = float(temp_c)        # from sidebar defaults
    precip_mm_h_run = float(precip_mm_h)
    if use_live_weather:
        try:
            mid_lon = (start[0] + end[0]) / 2.0
            mid_lat = (start[1] + end[1]) / 2.0
            temp_c_run, precip_mm_h_run = fetch_open_meteo_weather(mid_lat, mid_lon)
            st.info(f"🌦️ Live weather (Open-Meteo) @ route midpoint: {temp_c_run:.1f}°C, rain {precip_mm_h_run:.1f} mm/h")
        except Exception as e:
            st.warning(f"Live weather failed, using manual weather inputs. ({e})")

    # Recompute physics + battery factors for run temp
    rho_air = air_density_from_temp_c(temp_c_run)
    batt_cap_fac = battery_capacity_factor(temp_c_run) if use_battery_temp else 1.0
    batt_E_mult = battery_energy_multiplier(temp_c_run) if use_battery_temp else 1.0
    ...
    # If only start/end, build an interpolated straight line route like original
    if len(coords) == 2:
    ...
    if debug_mode:
        st.caption("[DEBUG] Route basics")
        st.json({"n_coords": len(coords), "n_elev": len(elevations), "length_m": float(length_m), "duration_s": float(duration_s)})
    ...
    # Steps / intersections / segmentation
    ...
    hvac_power_kw = 0.0
    if use_climate:
        hvac_power_kw = hvac_electric_power_kw(temp_c_run, climate_intensity, hvac_params)

    adjusted_aux_power = float(aux_power_kw) + float(hvac_power_kw)

    # Total mass
    total_mass_kg = float(mass_kg) + float(total_passenger_weight)

    # Rain factor → effective Crr
    rain_factor = rolling_resistance_rain_factor(precip_mm_h_run)
    crr_effective = float(crr) * float(rain_factor)
    ...
    for i, v in enumerate(candidates):
        ...
        try:
            ...
            results.append(
                dict(
                    speed=int(v),
                    energy_Wh=float(E_Wh),
                    time_h=float(T_h),
                    dist_km=float(D_km),
                    avg_speed=float(avg_speed),
                    energy_kwh=float(energy_kwh_candidate),
                    charge_info=charge_info_candidate,
                    charging_time_min=float(charging_time_min_candidate),
                    total_time_min=float(total_time_min_candidate),
                )
            )
    ...
    if minimize_target == "Minimize energy under time constraint":
        ...
        def norm(x, a, b):
            return 0.0 if a == b else (x - a) / (b - a)
    ...
    trip_co2_kg = energy_needed_kwh * (grid_co2_g_per_kwh / 1000.0)

    charging_time_min = best["charging_time_min"]
    best_driving_time_min = best["time_h"] * 60.0
    best_total_time_min = best["total_time_min"]

    # Battery at departure and computed arrival
    battery_start_kwh = veh["battery_kwh"] * (battery_start_pct / 100.0)
    battery_after_trip = battery_start_kwh - energy_needed_kwh
    battery_end_pct_calc = (battery_after_trip / veh["battery_kwh"]) * 100.0 if veh["battery_kwh"] > 0 else 0.0

    with st.expander("📋 Trip parameter summary", expanded=True):
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("👥 Passengers", f"{num_passengers}", help=f"Total weight: {total_passenger_weight} kg")
        c2.metric("🌡️ HVAC", "✅ Yes" if use_climate else "❌ No",
                  help=(f"Intensity: {climate_intensity}% | HVAC elec: {hvac_power_kw:.2f} kW" if use_climate else ""))
        c3.metric("🌦️ Weather", f"{temp_c_run:.1f}°C", help=f"Rain: {precip_mm_h_run:.1f} mm/h → Crr×{rain_factor:.2f}")
        c4.metric("🔋 Battery effective", f"{veh['battery_kwh']:.1f} kWh", help=f"Capacity factor: {batt_cap_fac:.2f} | Energy mult: {batt_E_mult:.2f}")
    ...
    colD.metric("Total time (incl. charges)", f"{best_total_time_min:.1f} min", help=f"Includes {charging_time_min:.1f} min of estimated charging")
    ...
    colR4.metric("Mean slope (abs)", f"{mean_abs_slope_pct:.2f} %")
    ...
    if use_segmented_speeds:
        st.success("✅ **Per-segment speed limits enabled**: Calculations consider limits by road type (motorway/city/etc.)")
        st.info(f"ℹ️ Actual average speed: {best['avg_speed']:.1f} km/h (base advised speed: {best['speed']} km/h)")
    ...
    colG.metric("🔌 Required charges", f"{charge_info['num_stops']}", help=f"{CHARGING_STOP_DURATION_MIN} min each (estimate)")
    ...
    if charge_info["num_stops"] == 0:
        st.success("✅ **No charging needed!** You have enough battery for this trip.")
    elif 0 < charge_info["num_stops"] < 10:
        st.warning(f"🔋 **{charge_info['num_stops']} charge(s) recommended** for this trip.")
    else:
        st.error(f"⚠️ **Challenging trip**: very high consumption ({charge_info['num_stops']} estimated charges).")
    ...
    if energy_needed_kwh > veh["battery_kwh"]:
        st.error("❌ Consumption exceeds (effective) battery capacity! Trip is not feasible.")
    ...
    if route_latlons:
        ...
        try:
            ...
            if debug_mode:
                st.warning(f"[DEBUG] Map display failed: {map_err}")

    # Savings vs fastest
    dE_Wh = best["energy_Wh"] - fastest["energy_Wh"]
    dT_min = best_total_time_min - fastest["total_time_min"]

    st.markdown("#### Impact vs fastest driving (among your candidate speeds)")
    cc1, cc2, cc3 = st.columns(3)
    cc1.metric("Energy saved", f"{-dE_Wh/1000:.2f} kWh" if dE_Wh < 0 else f"+{dE_Wh/1000:.2f} kWh")
    cc2.metric("Time added", f"{dT_min:.1f} min", help="Includes extra time due to estimated charges")
    fastest_co2_kg = float(fastest["energy_kwh"]) * (grid_co2_g_per_kwh / 1000.0)
    dco2 = trip_co2_kg - fastest_co2_kg
    cc3.metric("CO₂ vs fastest", f"{dco2:+.2f} kg")
    ...
    df = pd.DataFrame(
        [
            dict(
                ...
                Cost_EUR=float(r["energy_kwh"]) * float(energy_cost_per_kwh),
                CO2_kg=float(r["energy_kwh"]) * (grid_co2_g_per_kwh / 1000.0),
    ...
    feasible_speeds = set(r["speed"] for r in results if max_time_total_min is None or r["total_time_min"] <= max_time_total_min)
    ...
    with col_graph1:
        ...
        ax.scatter(best["speed"], best["energy_kwh"], s=220, c="#e74c3c", marker="*", edgecolors="black", linewidth=0.8, zorder=5)
        ax.annotate(
            f"Best: {best['speed']} km/h\n{best['energy_kwh']:.2f} kWh",
            xy=(best["speed"], best["energy_kwh"]),
            xytext=(15, 10),
    ...
    with col_graph2:
        ...
        ax.scatter(
            x, yT, s=100,
            c=["#2ecc71" if int(s) in feasible_speeds else "#e67e22" for s in x],
        ...
        ax.annotate(
            ...
            xy=(best["speed"], best_total_time_min),
            xytext=(15, -25),
            textcoords="offset points",
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#e74c3c", lw=1),
            arrowprops=dict(arrowstyle="->", color="#e74c3c"),
